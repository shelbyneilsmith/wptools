		...
		<?php } else { ?>
			...
			<?php
				if (get_post_meta( get_the_ID(), 'yb_source', true ) == 'vimeo') {
					echo '<div id="portfolio-video" class="ten columns alpha"><iframe src="http://player.vimeo.com/video/'.get_post_meta( get_the_ID(), 'yb_embed', true ).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="960" height="540" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
				} else if (get_post_meta( get_the_ID(), 'yb_source', true ) == 'youtube') {
					echo '<div id="portfolio-video" class="ten columns alpha"><iframe width="960" height="540" src="http://www.youtube.com/embed/'.get_post_meta( get_the_ID(), 'yb_embed', true ).'?rel=0&showinfo=0&modestbranding=1&hd=1&autohide=1&color=white" frameborder="0" allowfullscreen></iframe></div>';
				} else {
					echo '<div id="portfolio-video" class="ten columns alpha">'.get_post_meta( get_the_ID(), 'yb_embed', true ).'</div>';
				}
			?>
